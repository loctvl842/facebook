...
import { Close, PermMediaRounded, TagFacesOutlined, AddAPhoto } from "@mui/icons-material";
...
function CreatePost({ visible, setVisible, cardRef }) {
  ...
  const { error, isFetching } = useSelector((state) => state.post);
          ...
          >
            ...
            <div className="cardButtonWrapper">
              <button ref={postBtn} className="cardButton" type="submit" disabled={isFetching}>
                {isFetching ? <CircularProgress style={{ color: "#fff" }} size="25px" /> : "Post"}
              </button>
            </div>
            {error && (
